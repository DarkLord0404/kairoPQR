...
<nav x-data="{ open: false }" class="kairo-navbar">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex items-center gap-8">
                <a href="{{ route('reuniones') }}" wire:navigate class="flex items-center gap-3 shrink-0">
                    <img src="{{ asset('kairo.png') }}" alt="Kairo" class="w-9 h-9 kairo-avatar-ring">
                    <span class="font-bold text-sm tracking-wide" style="color: var(--kairo-text)">KAIRO MEET</span>
                </a>

                <div class="hidden space-x-6 sm:flex">
                    <a href="{{ route('reuniones') }}" wire:navigate
                       class="inline-flex items-center px-1 pt-1 text-sm kairo-navlink {{ request()->routeIs('reuniones') || request()->routeIs('reuniones.detalle') ? 'kairo-navlink-active' : '' }}">
                        {{ __('Reuniones') }}
                    </a>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('sso.ir-a-pqr') }}" class="kairo-btn-copy">Ir a PQR</a>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden" style="border-top: 1px solid var(--kairo-border)">
        ...
        <div class="pt-4 pb-1" style="border-top: 1px solid var(--kairo-border)">
            <div class="px-4">
                <div class="font-medium text-base" style="color: var(--kairo-text)">{{ auth()->user()->name }}</div>
