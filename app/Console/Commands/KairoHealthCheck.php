...
class KairoHealthCheck extends Command
{
    ...
    public function handle(): void
    {
        ...
        $this->verificar('Inteligencia Artificial', 'Análisis de texto (OpenAI)',    fn () => $this->checkOpenClaw());
    ...
    private function checkSmtp(): array
    {
        ...
        if (!$host) return ['advertencia', 'MAIL_HOST no configurado en .env'];
        $conn = @fsockopen($host, $port, $errno, $errstr, 5);
        if (!$conn) return ['fallo', "Sin conexión con {$host}:{$port} — los correos no se enviarán"];
        fclose($conn);
    ...
    private function checkDisco(): array
    {
        ...
        if (!is_dir($dir)) return ['fallo', 'Carpeta de grabaciones no existe en el servidor'];
        ...
        if ($pct > 90) $msg .= ' — DISCO CASI LLENO';
        elseif ($pct > 80) $msg .= ' — espacio reducido';
        return [$estado, $msg];
    }

    private function checkOpenClaw(): array
    {
        $result = Process::timeout(30)->run([
            'sudo', '-H', '-u', 'root',
            'openclaw', 'agent', '--agent', 'main',
            '--message', 'Responde solo: KAIRO_OK',
        ]);
        if (!$result->successful() || trim($result->output()) === '') {
            return ['fallo', 'Sin respuesta — los análisis PQR y EA no funcionarán: '.substr($result->errorOutput(), 0, 100)];
        }
        return ['ok', 'Motor de análisis respondiendo correctamente vía suscripción OpenAI'];
    }

    private function checkGroq(): array
    {
        $key = $this->groqApiKey();
        if (!$key) return ['fallo', 'Clave de API no configurada — la transcripción de audio no está disponible'];
    ...
    private function checkGoogleToken(string $cuenta): array
    {
        ...
        if (!file_exists($path)) return ['fallo', 'No hay token guardado — necesita re-autorización en GCP'];
        $data         = json_decode(file_get_contents($path), true);
        $refreshToken = $data['refresh_token'] ?? '';
        if (!$refreshToken) return ['fallo', 'Token incompleto — necesita re-autorización'];
        ...
        $age = isset($data['obtenido_en'])
            ? now()->diffForHumans(\Carbon\Carbon::parse($data['obtenido_en']), true)
            : '?';
    ...
    private function groqApiKey(): string
    {
        ...
        if (!file_exists($env)) return env('GROQ_API_KEY', '');
